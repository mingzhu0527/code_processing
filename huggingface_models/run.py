...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        source = tokenizer(example.source, max_length=args.max_source_length, padding="max_length", truncation=True)
        source_ids, source_mask = source.input_ids, source.attention_mask
        if stage=="test":
            target_ids, target_mask = None, None
        target = tokenizer(example.target, max_length=args.max_target_length, padding="max_length", truncation=True)
...
def convert_examples_to_features1(examples, tokenizer, args,stage=None):
    features = []
    for example_index, example in enumerate(examples):
        #source
